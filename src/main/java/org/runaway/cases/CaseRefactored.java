...
public interface CaseRefactored {
    ...
    default void open(Gamer gamer) {
        ...
        caseMenu.setItem(4, new ItemBuilder(Material.STAINED_GLASS_PANE).name("&aНаграда").durability(4).build());
        caseMenu.setItem(22, new ItemBuilder(Material.STAINED_GLASS_PANE).name("&aНаграда").durability(4).build());
        for (int i = 0; i < 27; i++) {
            if (i < 10 || i > 16) {
                if (caseMenu.getButton(i) == null)
                    caseMenu.setItem(i, new ItemBuilder(Material.STAINED_GLASS_PANE).name("&r").durability(5).build());
            }
        }
        for (int i = 1; i < 8; i++) {
