        ...
        return (itemStack -> {
            ...
            for (int i = 1; i < (getAmount() + 1); i++) {
                if (getDefaultRunes() != null && getDefaultRunes().size() > (i - 1)) {
                    ...
                    ItemUtils.addLore(itemStack, String.format(parameterManager.getRunesAmountString(), i, rune1.getName()));
                } else {
                    ItemUtils.addItemTag(itemStack, "rune" + i, "");
                    ItemUtils.addLore(itemStack, String.format(parameterManager.getRunesAmountString(), i, "-"));
                }
            }
            return itemStack;
        });
