    ...
    public static void updateMine(Mine mine) {
        mineTask = new BukkitRunnable() {
            public void run() {
                ...
                try {
                    Location newLoc = loc.clone().subtract(mine.getDiametr() - 1, mine.getHeight() - 1, mine.getDiametr() - 1);
                    Location oloc = loc.clone().subtract(0, isSurface ? 1 : 0, 0);
                    CuboidRegion cuboidSelection = new CuboidRegion(BukkitAdapter.adapt(loc.getWorld()),
                            BlockVector3.at(oloc.getX(), oloc.getY(), oloc.getZ()),
                            BlockVector3.at(newLoc.getX(), newLoc.getY(), newLoc.getZ()));
                    try (EditSession editSession = WorldEdit.getInstance().newEditSession(cuboidSelection.getWorld())) {
